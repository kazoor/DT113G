...
while True:
    ...
    try:
        ...
        data_received = message.decode().split('\r\n')
        ...
        rows = data_received[1:]

        # Try to loop through all rows
        try:
            for row in rows:
                ...
                if "gasbot_sepia.png" in requested_resource:
                    f = open("./pages/" +
                             requested_resource[1:], 'rb')
                    output = f.read()
                    f.close()
                    connectionSocket.sendall(output)
                else:
                    if "Firefox" in row:
                        f = open("./pages/text_only.html", 'rb')
                        output = f.read()
                        f.close()
                        connectionSocket.sendall(output)

                    elif "Trident" in row:
                        f = open("./pages/text_image.html", 'rb')
                        output = f.read()
                        f.close()
                        connectionSocket.sendall(output)
